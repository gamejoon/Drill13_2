...
class Zombie:
    ...
    def ball_count_better_than_and_is_boy_nearby(self):
        if self.ball_count > play_mode.boy.ball_count and self.distance_less_than(play_mode.boy.x, play_mode.boy.y, self.x, self.y, 7):
            return BehaviorTree.SUCCESS
        else:
            return BehaviorTree.FAIL
    ...
    def build_behavior_tree(self):
        ...
        c2 = Condition('소년의 공 갯수보다 많은가?', self.ball_count_better_than_and_is_boy_nearby)
        SEQ_chase_boy_when_ball_count_better_than = Sequence('소년보다 공 갯수가 많을 때 추적', c2, a4)
        root = SEL_chase_when_ball_count_better_than_or_wander = Selector('소년보다 공 갯수가 많을 때 추적 또는 배회', SEQ_chase_boy_when_ball_count_better_than, SEQ_wander)
